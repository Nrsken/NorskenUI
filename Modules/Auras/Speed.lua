...
---@class SpeedModule
local Speed = NRSKNUI:GetModule('Speed')
...
function Speed:UpdateDB()
    self.db = NRSKNUI.db.profile.Speed
end

function Speed:OnInitialize()
    self:UpdateDB()
...
---@return table
function Speed:GetContainerConfig()
    local db = self.db
...
---@return number
function Speed:GetPreviewCount()
    local db = self.db
    return min(db.maxFrameCount * NRSKNUI:GetAuraFilterBranchCount(db.Filter), db.previewLimit)
end

---Size the host to the largest grid the current settings can produce, so the mover covers the display's full footprint.
function Speed:ResizeHost()
...
function Speed:CreateHost()
    if self.host then return end

    self.host = CreateFrame('Frame', 'NRSKNUI_Speed', UIParent)
    self:ResizeHost()
end

---Attach the native aura container to a host.
function Speed:BuildContainer()
...
function Speed:OnFilterChanged()
