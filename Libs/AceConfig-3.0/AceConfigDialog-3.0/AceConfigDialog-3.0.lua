...
-- NRSKNA: Bumped version to 9001 to ensure our multiselect sorting patch is always used
-- (Other addons typically use version 89-90)
local MAJOR, MINOR = "AceConfigDialog-3.0", 9001
		...
		if not hidden then
			if v.type == "group" then
				...
				elseif v.type == "multiselect" then
					...
					-- NRSKNA: Support sorting property for multiselect (same as select type)
					local sorting = GetOptionsMemberValue("sorting", v, options, path, appName)
					local valuesort = new()
					if sorting and values then
						-- Use explicit sorting order if provided
						for _, value in ipairs(sorting) do
							if values[value] ~= nil then
								tinsert(valuesort, value)
							end
						end
						-- Add any values not in sorting (fallback for completeness)
						local added = {}
						for i = 1, #valuesort do added[valuesort[i]] = true end
						for value in pairs(values) do
							if not added[value] then
								tinsert(valuesort, value)
							end
						end
					elseif values then
